...
class ModelBase:
    ...
    def dequant_model(self):
        ...
        if self._is_nvfp4 and not any(
            k.endswith((".weight_scale", ".weight_scale_inv"))
        ...
        if (quant_config := self.hparams.get("quantization_config")) and isinstance(
            ...
            if quant_method == "bitnet":
                ...
                for name in self.model_tensors.keys():
                    ...
                    if name.endswith(
                        "_activation_scale"
                    ):  # Mistral-Small-4-119B-2602, unused
                        tensors_to_remove.append(name)
                    # mistral format
    ...
    def _nvfp4_pack(weight: Tensor, scale: Tensor) -> tuple[np.ndarray, list[int]]:
        ...
        vals = torch.stack([w & 0x0F, w >> 4], dim=-1).reshape(
            out_features, n_blocks, 16
        )
        ...
        qs_grouped = qs.reshape(out_features, n_super, 4, 8).reshape(
            out_features, n_super, 32
        )
        raw = np.concatenate([d_grouped, qs_grouped], axis=-1).reshape(
            out_features, n_super * 36
        )
    ...
    def _repack_nvfp4(
        self, new_name: str, weight: Tensor, scale: Tensor, scale2: Tensor
    ):
        raw, shape = self._nvfp4_pack(weight, scale)
        logger.info(f"Repacked {new_name} with shape {shape} and quantization NVFP4")
        self.gguf_writer.add_tensor(
            new_name, raw, raw_dtype=gguf.GGMLQuantizationType.NVFP4
        )
        ...
        if not self._nvfp4_scale2_is_trivial(scale2):
            ...
            logger.info(
                f"  + {scale_name} (per-tensor NVFP4 scale2, shape [{scale2_f32.size}])"
            )
    ...
    def _generate_nvfp4_tensors(self):
        ...
        n_experts = (
            self.find_hparam(["num_local_experts", "num_experts"], optional=True) or 0
        )
        consumed: list[str] = []
        ...
        for name in list(self.model_tensors.keys()):
            ...
            scale2 = LazyTorchTensor.to_eager(
            ...
            m = re.search(
                r"\.experts\.(\d+)\.(gate_proj|up_proj|down_proj)\.weight$", name
            )
            if m:
                ...
                if n_experts > 0 and len(expert_blocks[key]) >= n_experts:
                    self._flush_nvfp4_experts(
                        key, expert_blocks, expert_scales, expert_shapes, bid, proj_type
                    )
            else:
                new_name = self.map_tensor_name(name)
    ...
    def _flush_nvfp4_experts(
        self, key, expert_blocks, expert_scales, expert_shapes, bid, proj_type
    ):
        ...
        logger.info(
            f"Repacked {new_name} with shape [{len(experts)}, {shape[0]}, {shape[1]}] and quantization NVFP4"
        )
        self.gguf_writer.add_tensor(
            new_name, merged, raw_dtype=gguf.GGMLQuantizationType.NVFP4
        )
        ...
        if not np.allclose(scale_vals, 1.0, atol=1e-6):
            scale_name = new_name.replace(".weight", ".scale")
            logger.info(
                f"  + {scale_name} (per-expert NVFP4 scale2, shape [{len(scales)}])"
            )
    ...
    def prepare_tensors(self):
        ...
        quant_layers = (self.hparams.get("quantization_config") or {}).get(
            "quantized_layers"
        ) or {}
        ...
        if quant_algo != "NVFP4":
            if any(
                v.get("quant_algo") == "NVFP4"
                for v in quant_layers.values()
    ...
    def modify_tensors(
        ...
        if name.startswith("visual."):
            ...
            if ".qkv." in name:
                ...
                assert kt == 2, (
                    "Current implementation only support temporal_patch_size of 2"
                )
                yield (
        ...
        if self._is_nvfp4 and "experts" in name:
            if name.endswith(
                ...
                if (
                    ...
                    and name.replace(".weight", ".weight_scale") in self.model_tensors
                ):
                    return
                if not name.endswith(".weight"):
                    return
...
class Qwen3Model(Qwen2Model):
    ...
    def _is_qwen3_reranker(self) -> bool:
        ...
        if (
            "# qwen3-reranker" in readme_text.lower()
            or "# qwen3-vl-reranker" in readme_text.lower()
        ):
            return True

        if any(
            "qwen3-reranker" in hint or "qwen3-vl-reranker" in hint
...
class Phi4VisionMmprojModel(MmprojModel):
    def __init__(self, *args, **kwargs):
        ...
        if patch_size is None:
            raise KeyError(
                "Phi-4 vision mmproj conversion requires patch_size in preprocessor_config.json"
            )
        ...
        if pos_emb_name is None:
            raise KeyError(
                "Phi-4 vision mmproj conversion could not find position_embedding.weight"
            )
        ...
        if grid_side * grid_side != base_grid_tokens:
            raise ValueError(
                f"Unexpected Phi-4 position embedding shape: {tuple(pos_emb_shape)}"
            )

        self.hparams_vision["image_size"] = grid_side * patch_size

        min_num_patches = self.preprocessor_config.get(
            "min_num_patches", self.global_config.get("min_num_patches")
        )
        max_num_patches = self.preprocessor_config.get(
            "max_num_patches", self.global_config.get("max_num_patches")
        )
        if min_num_patches is None or max_num_patches is None:
            raise KeyError(
                "Phi-4 vision mmproj conversion requires min_num_patches and max_num_patches"
            )
        ...
        if name.startswith(("model.vision_tower.vision_tower.", "vision_tower.")):
            ...
            if new_name.endswith("vision_model.embeddings.patch_embedding.weight"):
                ...
                if data_torch.ndim != 2:
                    raise ValueError(
                        f"Unexpected Phi-4 patch embedding shape: {tuple(data_torch.shape)}"
                    )
                ...
                data_torch = data_torch.view(
                    data_torch.shape[0], patch_size, patch_size, num_channels
                )
        ...
        if name.startswith(("model.mm_projector.", "mm_projector.")):
            ...
            yield (
                self.format_tensor_name(
                    gguf.MODEL_TENSOR.V_MMPROJ, mm_idx, suffix=suffix
                ),
                data_torch,
            )
            return
...
class NemotronHModel(GraniteHybridModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        pattern = self.hparams.get("hybrid_override_pattern") or self.hparams.get(
            "layers_block_type"
        )
        if pattern is None:
            self._ssm_layers = []
    ...
    def get_attn_layers(self):
        ...
        if pattern is None:
            return []
        assert len(pattern) == self.block_count, (
